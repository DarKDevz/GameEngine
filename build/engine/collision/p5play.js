...
p5.prototype.registerMethod('init', function p5PlayInit() {
    ...
    this.Sprite = class {
        ...
        constructor(x, y, w, h, collider) {
            ...
            Object.defineProperties(this._vel, {
                x: {
                    get() {
                        let val;
                        if (_this.body)
                            val = _this.body.getLinearVelocity().x;
                        else
                            val = _this._velocity.x;
                ...
                y: {
                    get() {
                        let val;
                        if (_this.body)
                            val = _this.body.getLinearVelocity().y;
                        else
                            val = _this._velocity.y;
                        return fixRound(val / _this.tileSize);
                    },
                    set(val) {
                        val *= _this.tileSize;
            ...
            if (this.collider != 'none') {
                ...
                this.h = h || this.w;
                if (Array.isArray(w)) {
                    throw new Error('Cannot set the collider type of a sprite with a polygon or chain shape to "none". Try having the sprite overlap with other sprites instead.');
                }
            ...
            if (typeof gvx == 'function')
                gvx = gvx(group.length - 1);
            if (typeof gvy == 'function')
                gvy = gvy(group.length - 1);
            this.vel.x = gvx;
            this.vel.y = gvy;
            ...
            this.color ??= this.p.color(Math.round(this.p.random(30, 245)), Math.round(this.p.random(30, 245)), Math.round(this.p.random(30, 245)));
            ...
            if (shouldCreateSensor && !this._hasSensors)
        ...
        _parseShape(offsetX, offsetY, w, h) {
            ...
            if (shape == 'box') {
                ...
                if (isSlop(Math.abs(vecs[0].x) - Math.abs(vecs[vecs.length - 1].x)) &&
        ...
        set collider(val) {
            ...
            if (val == 'none' && (this._shape == 'chain' || this._shape == 'polygon')) {
                throw new Error('Cannot set the collider type of a polygon or chain collider to "none". Try having the sprite overlap with other sprites instead.');
            }
            if (this.watch)
                this.mod[10] = true;
        ...
        set mirror(val) {
            if (this.watch)
                this.mod[22] = true;
            if (val.x !== undefined)
                this._mirror.x = val.x;
            if (val.y !== undefined)
                this._mirror.y = val.y;
        }
        /**
        ...
        set offset(val) {
            ...
            if (val.x == this._offset._x && val.y == this._offset._y)
                return;
            if (this.watch)
                this.mod[23] = true;
        ...
        set scale(val) {
            ...
            if (val.x == this._scale._x && val.y == this._scale._y)
                return;
            if (this.watch)
                this.mod[28] = true;
            let scalars = {
        ...
        _getVertices(output2DArrays) {
            ...
            for (let i = 0; i < v.length; i++) {
                ...
                if (output2DArrays)
                    ...
                    v[i] = pInst.createVector(arr[0], arr[1]);
            }
            return v;
        }
        // TODO set vertices
        ...
        _step() {
            ...
            if (this.watch) {
                if (this.x != this.prevX)
                    this.mod[0] = this.mod[2] = true;
                if (this.y != this.prevY)
                    this.mod[1] = this.mod[2] = true;
            ...
            for (let event in eventTypes) {
                for (let k in this[event]) {
                    ...
                    let v = a[event][k] + 1;
                    this[event][k] = v;
                    if (b instanceof this.p.Group)
                        b[event][a._uid] = v;
                    if (!b || v == 0) {
                        delete a[event][k];
                        if (b instanceof this.p.Group)
                            delete b[event][a._uid];
                        continue;
                    }
                    else if (v == -1) {
        ...
        _display() {
            ...
            if (this._pixelPerfect) {
                if (this._w % 2 == 0 || !isSlop((x % 1) - 0.5))
                    x = Math.round(x);
                if (this._h % 2 == 0 || !isSlop((y % 1) - 0.5))
                    y = Math.round(y);
            }
        ...
        moveTowards(x, y, tracking) {
            ...
            if (typeof x != 'number') {
                let obj = x;
                if (obj == this.p.mouse && !this.p.mouse.active)
                    return;
        ...
        move(distance, direction, speed) {
            ...
            if (dirNameMode) {
                ...
                if (distance !== undefined) {
                    console.warn(`In p5play v3.3.0 the parameter ordering for the move() function was changed to: move(distance, direction, speed).`);
                }
            }
            else {
        ...
        rotateTowards(angle, tracking) {
            ...
            if (x !== undefined)
                angle = this.angleToFace(x, y, facing);
            else {
                angle -= this.rotation;
            }
            tracking ??= 0.1;
        ...
        rotate(angle, speed) {
            if (this.__collider == 1)
                throw new FriendlyError(0);
            if (isNaN(angle))
                throw new FriendlyError(1, [angle]);
            if (angle == 0)
                return;
            ...
            return (async () => {
                if (frames > 1) {
                    ...
                    do {
                        if (this._rotateIdx != _rotateIdx)
                            return false;
                        await pInst.delay();
                        if ((cw && this.rotationSpeed < 0.01) || (!cw && this.rotationSpeed > -0.01)) {
                            return false;
                        }
                    } while (((cw && ang > this.rotation) || (!cw && ang < this.rotation)) &&
        ...
        async changeAni(anis) {
            ...
            for (let i = 0; i < anis.length; i++) {
                let ani = anis[i];
                if (ani instanceof this.p.SpriteAnimation ||
        ...
        _playSequencedAni(ani) {
            return new Promise((resolve) => {
                ...
                if (flipX)
                    this._ani.scale.x = -this._ani.scale.x;
                if (flipY)
                    this._ani.scale.y = -this._ani.scale.y;
                if (start < 0)
                    start = this._ani.length + start;
                if (start !== undefined)
                    this._ani.frame = start;
                if (end !== undefined)
                    this._ani.goToFrame(end);
                else if (this._ani.frame == this._ani.lastFrame)
                    resolve();
         ...
         */
        _changeAni(label) {
            if (this._ani?._onChange)
                this._ani._onChange();
            if (this._ani?.onChange)
                this._ani.onChange();
            ...
            if (this.resetAnimationsOnChange)
                this._ani.frame = 0;
        }
        /**
        ...
        constructor() {
            ...
            // SpriteSheet mode
                ...
                function _generateSheetFrames() {
                    ...
                    let { w, h, width, height, size, row, col, line, x, y, pos, frames, frameCount, frameDelay, frameSize, delay, rotation } = atlas;
                    frameSize ??= size || owner.anis.frameSize;
                    if (delay)
                        _this.frameDelay = delay;
                    if (frameDelay)
                ...
                for (let i = 0; i < args.length; i++) {
                    if (args[i] instanceof p5.Image)
                        this.push(args[i]);
                    else
                        this.push(this.p.loadImage(args[i]));
                }
            }
        }
        /**
            ...
            if (img !== undefined) {
                if (this.spriteSheet) {
                    ...
                    if (!this.demoMode) {
                        ...
                        this.p.image(this.spriteSheet, this.offset.x, this.offset.y, this.spriteSheet.w, this.spriteSheet.h, x - this.spriteSheet.w / 2 + w / 2, y - this.spriteSheet.h / 2 + h / 2);
                    }
                }
                else {
            ...
            for (let prop of props) {
                Object.defineProperty(this, prop, {
                    ...
                    set(val) {
                        ...
                        for (let k in _this) {
                            let x = _this[k];
                            if (!(x instanceof SpriteAnimation))
                                continue;
                            x[prop] = val;
                        }
                    }
    ...
    this.Group = class extends Array {
        ...
        constructor(...args) {
            ...
            for (let vecProp of vecProps) {
                ...
                if (vecProp != 'vel')
                    ...
                    this[vecProp] = new this.p.Vector();
                this[vecProp]._x = 0;
                this[vecProp]._y = 0;
                for (let prop of ['x', 'y']) {
        ...
        _getTopLayer() {
            if (this.length == 0)
                return 0;
            if (this.length == 1 && this[0]._layer === undefined)
                return 0;
        ...
        push(...sprites) {
            for (let s of sprites) {
                ...
                super.push(s);
                if (this.parent)
                    this.p.p5play.groups[this.parent].push(s);
                s.groups.push(this);
            }
            return this.length;
        }
    ...
    this.Joint = class {
        ...
        constructor(spriteA, spriteB, type) {
            ...
            let _this = this;
            for (let l of ['A', 'B']) {
                if ((type == 'revolute' || type == 'wheel') && l == 'B')
                    break;
        ...
        _display() {
            if (this.type == 'distance') {
                this._draw(this.spriteA.x + this.offsetA.x, this.spriteA.y + this.offsetA.y, this.spriteB.x + this.offsetB.x, this.spriteB.y + this.offsetB.y);
            }
    ...
    this.DistanceJoint = class extends this.Joint {
        ...
        constructor(spriteA, spriteB) {
            ...
            let j = pl.DistanceJoint({}, spriteA.body, spriteB.body, spriteA.body.getWorldCenter(), spriteB.body.getWorldCenter());
            this._createJoint(j);
        }
    };
    this.WheelJoint = class extends this.Joint {
        ...
        constructor(spriteA, spriteB) {
            ...
            let j = pl.WheelJoint({
                ...
                frequencyHz: 4,
                dampingRatio: 0.7
            }, spriteA.body, spriteB.body, spriteB.body.getWorldCenter(), new pl.Vec2(0, 1));
            this._createJoint(j);
        }
    };
    ...
    this.PrismaticJoint = class extends this.Joint {
        constructor(spriteA, spriteB) {
            ...
            let j = pl.PrismaticJoint({
                ...
                motorSpeed: 0,
                enableMotor: true
            }, spriteA.body, spriteB.body, spriteA.body.getWorldCenter(), spriteB.body.getWorldCenter());
            this._createJoint(j);
        }
        /**
    ...
    this.Netcode = class {
        ...
        binaryToSprite(binary, offset) {
            ...
            if (binary instanceof DataView)
                ...
                data = new DataView(binary.buffer);
    ...
    function getRegularPolygon(l, n) {
        if (n == 'triangle')
            ...
            l = [l, -36, 10];
        else if (n == 'hendecagon')
            l = [l, -32.7272727273, 11];
        else if (n == 'dodecagon')
            l = [l, -30, 12];
        return l;
    }
    ...
    this.play = (sound) => {
        if (!sound.play)
            throw new Error('Tried to play a sound but the sound is not a sound object: ' + sound);
    ...
    this.p5play.playIntro = async function () {
        if (document.getElementById('p5play-intro'))
            return;
...
canvas {
    ...
    this.loadImg = this.loadImage = function () {
        ...
        if (img) {
            ...
            if ((img.width == 1 && img.height == 1) || !img.pixels.length) {
                ...
                if (cb)
                    cb(); // if already loaded, run the callback immediately
                pInst._decrementPreload();
            }
            return img;
        }
    ...
    class FriendlyError extends Error {
        constructor(func, errorNum, e) {
            ...
            if (m.base)
                ...
                msg = errMsgs.generic[Math.floor(Math.random() * errMsgs.generic.length)] + ln;
    ...
    this._Mouse = class extends this.InputDevice {
        ...
        ac(inp) {
            if (inp.slice(0, 4))
                inp = 'left';
            else if (inp.slice(0, 5) == 'right')
                inp = 'right';
            else if (inp.slice(0, 6) == 'middle')
                inp = 'center';
            else
                inp = inp.toLowerCase();
            return inp;

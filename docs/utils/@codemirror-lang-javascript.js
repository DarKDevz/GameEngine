...
import { snippetCompletion, ifNotIn, completeFromList, closeBrackets } from "./@codemirror-autocomplete.js";
...
const keywords = /*@__PURE__*/"break case const continue default delete export extends false finally in instanceof let new return static super switch throw true typeof var yield".split(" ").map(kw => ({ label: kw, type: "keyword" }));
/**
JavaScript support. Includes [snippet](https://codemirror.net/6/docs/ref/#lang-javascript.snippets)
completion.
*/
function javascript(config = {}, customObj) {
...
function completeThisOutside(context) {
    let nodeBefore = syntaxTree(context.state).resolveInner(context.pos, -1)
    let textBefore = context.state.sliceDoc(nodeBefore.from, context.pos)
    if(textBefore.length < 5) {
        let toMatch = ["t","h","i","s"]
        for(let i = 0; i < textBefore.length; i++) {
            if(toMatch[i]!==textBefore[i]) {
                return null;
            }
        }
        let blockBefore = context.state.sliceDoc(0,nodeBefore.from)
        let howManyCloses = 0
        for(let i = blockBefore.length; i>=0; i--) {
            let char = blockBefore[i]
            if(char === "}") {
                howManyCloses++
            }else if(char === "{") {
                howManyCloses--;
        ...
        return (ifNotIn(dontComplete,completeFromList([snippetCompletion("this.${name} = function (${params}) {\n\t${}\n}", {
            label: "this.",
            detail: "definition",
            type: "keyword"
        })]))(context))
    }
    console.log(nodeBefore,textBefore);
    return null;
  }
